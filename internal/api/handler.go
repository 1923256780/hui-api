...
// Handler 是管理面 API 处理器集合（按域拆分文件，共享 st 与 sess）。
type Handler struct {
	st   *store.Store
	sess *SessionManager
}
...
// New 构造管理面处理器。
func New(st *store.Store, sess *SessionManager) *Handler {
	return &Handler{st: st, sess: sess}
}
...
// registerManaged 挂载 root 权限的管理端点（渠道/令牌/用户/兑换码/日志/配置）。
// 本提交先落会话与中间件骨架，端点在后续提交按域填充。
