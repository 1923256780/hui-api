...
- 端口决策（2026-09-04 用户拍板）：**hui-api 长期使用 3100 端口，取消原计划「改绑 3000」
  切换窗口**——docs/06 第七节 runbook 改写为「启用 3100 为生产」最终形态：保留预检/快照/
  最终迁移（重置空库）/冒烟/观察，删除停旧网关与改绑 3000 步骤，回滚 = 流量入口切回
  旧网关（RTO<30s，旧库全程零写入）；新增数据分叉警示节（旁路库数据落后旧库 ≤24h
  （cron 周期），启用前必须重置空库重跑 migrate 终迁——秒级幂等，守卫已就位需空库或
  -allow-live-target）；docs/06 第一/三/五/六节端口表述与 scripts/deploy-smoke-server.sh
  头部注释同步；docs/10 ROADMAP M4 行、状态块 M4-wave3 条目与交接记录更新。
...
- AGENTS.md 索引化重构：升级为全部文档的唯一索引中枢与管理入口——文档地图（docs/00~11
  一行职责 + 全部 ADR（0001~0009）一行摘要 + scripts 关键脚本速查，全部附相对路径链接）、
  L1~L4 文档层级与修改权限、自回归维护规则（登记义务/代码为准/冷启动流程/收尾检查/
  禁止复制）；新增端口决策铁律（3100 长期端口）；新增 docs/07-工具与脚本索引.md
  （六个脚本用途/用法/使用约定，编号按 docs 顺延规则取 07）；docs/INDEX.md 收敛为
  指引壳（唯一索引收敛至 AGENTS.md §2，防双索引漂移）；docs/MAINTENANCE.md 收尾清单
  增补索引一致性项并同步章节引用；docs/02 引用同步。
