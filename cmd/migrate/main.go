...
func main() {
	...
	flag.Usage = func() {
		...
		flag.PrintDefaults()
		fmt.Fprintln(os.Stderr, "退出码: 0 成功且对账一致；1 迁移失败/对账不一致/报告落盘失败；2 参数错误")
	}
	flag.Parse()
	...
	if err := run(*legacy, *target, *report, *exportTokens); err != nil {
...
// 迁移失败（含对账不一致）也尽力落盘报告供人工排查；返回首个错误。
func run(legacyPath, targetPath, reportPath, exportTokensPath string) error {
	rep, err := migrate.Run(migrate.Options{LegacyPath: legacyPath, TargetPath: targetPath})
...
func printSummary(rep *migrate.Report) {
	...
	fmt.Printf("守卫: quota_per_unit=%d 缺省放行=%v\n", rep.Guard.QuotaPerUnit, rep.Guard.QuotaPerUnitIsDefault)
